...
import yori_funnel.DXL.DXLManager2 as DM
...
if __name__ == "__main__":
    ...
    XMDXL.operate_motors_abs(all, target, 100)
    XMDXL.initialize()
    # XMDXL.operate_motors_abs_rel(all, target, 50)
    XMDXL.read_motors(all)
    XMDXL.operate_motors_abs_rel(all, np.array([90]), 50)
    XMDXL.read_motors(all)
    # XMDXL.operate_motors_relative(tilt, np.array([90]), 50)
    # XMDXL.read_motors(all)
    # XMDXL.operate_motors_relative(tilt, np.array([90]), 50)
    # XMDXL.read_motors(all)
    # XMDXL.operate_motors_relative(tilt, np.array([-90]), 50)
    # XMDXL.operate_motors(tilt, np.array([720]), 200)
    # time.sleep(5)
