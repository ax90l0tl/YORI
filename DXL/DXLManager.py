...
class Dynamixel(object):
    def __init__(self, baudrate=115200, devicename='/dev/ttyUSB0', control_table=XM, protocol_version=1.0): #type (Dynamixel, dict) -> None
...
# -------------------------------------------------------------------
    ...
    def initialize(self, num_vp):
        init_theta_0 = np.array([0]) # in degrees
        #init_theta_1 = np.array([206.2]) # in degrees
        self.operate_motor_single(DXL_ID[0],init_theta_0, num_vp)
    ...
    def vps_gen_js(self, start_pos, end_pos, num_vp):
        ...
        for i in range(len(DXL_ID)):
            interval.append((end_pos[i]-start_pos[i])/num_vp)
        
        vps_js = np.zeros([num_vp,len(DXL_ID)])

        for i in range(len(DXL_ID)):
            for j in range(num_vp):
                vps_js[j][i] = start_pos[i]+(j+1)*interval[i]
    ...
    def vps_gen_js_single(self, start_pos, end_pos, num_vp):
        ...
        """
        interval = []
        interval.append((end_pos[0]-start_pos[0])/num_vp)
        
        vps_js = np.zeros([num_vp,1])
    ...
    def operate_motor_single(self, id, goal_theta, num_vp):
        present_theta = self.read_motors()[0]
        ...
        vps = self.vps_gen_js_single(present_theta, new_goal_theta, num_vp)
        print(vps)
        ...
        while index_step < len(vps):
            
            # print(vps[index_step])
            self.write('GOAL_POSITION', zip([id], [int(vps[index_step])]))
    ...
    def operate_motors(self, target_theta, num_vp):
        present_theta = self.read_motors()
        ...
        while index < len(vps):
            # print(index)
            # print(vps[index])
            self.write('GOAL_POSITION', zip(DXL_ID, [int(vps[index])]))
    ...
    def read_motors(self):
        read_th = self.read('PRESENT_POSITION', DXL_ID)
        # print(read_th)
        ...
        current_pos_0 = current_pos_0 * position_constant
        # current_pos_1 = current_pos_1 * position_constant
        print("current", current_pos_0)
        return np.array([current_pos_0])#, current_pos_1])
